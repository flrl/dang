...
int array_init(array_t *self) {
    ...
    if (NULL != (self->m_items = calloc(array_initial_reserve_size, sizeof(anon_scalar_t)))) {
...
int array_reserve(array_t *self, size_t new_size) {
    ...
    if (self->m_allocated_count < new_size) {
        anon_scalar_t *new_items = calloc(new_size, sizeof(anon_scalar_t));
        if (NULL != new_items) {
            memcpy(new_items, self->m_items, self->m_count * sizeof(anon_scalar_t));
            free(self->m_items);
...
int array_item_at(array_t *self, size_t index, anon_scalar_t *result) {
    assert(self != NULL);
...
int array_push(array_t *self, const anon_scalar_t *value) {
    assert(self != NULL);
    if (self->m_count == self->m_allocated_count) {
        anon_scalar_t *tmp = self->m_items;
        self->m_items = calloc(self->m_allocated_count + array_grow_size, sizeof(anon_scalar_t));
...
int array_unshift(array_t *self, const anon_scalar_t *value) {
    assert(self != NULL);
    assert(value != NULL);
    if (self->m_count == self->m_allocated_count) {
        anon_scalar_t *tmp = self->m_items;
        ...
        memcpy(&self->m_items[1], tmp, self->m_count * sizeof(anon_scalar_t));
        self->m_count++;
        free(tmp);
        return 0;
    }
    else if (self->m_count > 0) {
        memmove(&self->m_items[1], &self->m_items[0], self->m_count * sizeof(anon_scalar_t));
        anon_scalar_clone(&self->m_items[0], value);
        self->m_count++;
        return 0;
    }
...
int array_pop(array_t *self, anon_scalar_t *result) {
    assert(self != NULL);
...
int array_shift(array_t *self, anon_scalar_t *result) {
    assert(self != NULL);

    if (self->m_count > 0) {
        anon_scalar_t value = self->m_items[0];
        --self->m_count;
        memmove(&self->m_items[0], &self->m_items[1], self->m_count * sizeof(anon_scalar_t));
        if (result != NULL) {
